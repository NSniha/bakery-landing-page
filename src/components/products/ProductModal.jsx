...
const ProductModal = ({ product, isAdded, onAdd, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 px-4 py-6 backdrop-blur-[5px]" onMouseDown={onClose}>
      ...
      <div className="product-modal relative w-full max-w-[760px] overflow-hidden rounded-[14px] bg-[#f8f5ef] shadow-[0_30px_100px_rgba(0,0,0,0.4)]" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="grid md:grid-cols-[0.95fr_1.05fr]">
          <div className="flex min-h-[280px] items-center justify-center bg-[#171717] p-8 sm:min-h-[340px]">
            <img src={product.image} alt={product.name} className="max-h-[275px] max-w-full object-contain drop-shadow-[0_18px_22px_rgba(0,0,0,0.35)]" />
          </div>
          ...
          <div className="flex flex-col justify-center p-6 sm:p-9 lg:p-11">
            ...
            <div className="mt-7 flex items-center justify-between gap-5">
              ...
              <button type="button" disabled={isAdded} onClick={() => onAdd(product)} className={`min-h-[52px] rounded-[5px] px-7 text-[15px] font-medium transition-all duration-300 ${isAdded ? "cursor-not-allowed bg-[#ded9d0] text-[#8e8a83]" : "bg-[#a9432c] text-white hover:-translate-y-1 hover:bg-[#ba4a30] hover:shadow-[0_10px_24px_rgba(169,67,44,0.25)]"}`}>{isAdded ? "Added to Basket" : "Add to Basket"}</button>
            </div>
          </div>
        </div>
      </div>
